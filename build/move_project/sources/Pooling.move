module my_addrx::Pooling { 

    use 0x1::signer;
    use std::account;
    /// Error codes

    struct Pool has store,key,copy{
        viewPoint: vector<u8>,
        totalVotes:u64,
        trueVotes: u64
    }
    ...
    public fun assert_is_initialized(addr: address) {
        assert!(exists<Pool>(addr), 1);
    }

    public fun assert_uninitialized(addr: address) {
        assert!(!exists<Pool>(addr), 3);
    }   
    ...
    public fun vote(acc_own: &signer, store_addr: address,_vote:bool )acquires Pool{
        let addr = signer::address_of(acc_own);
        
        assert_uninitialized(addr);

        let op_store = borrow_global_mut<Pool>(store_addr);
    ...
    public fun currentStandings(store_addr: address):u64 acquires Pool{
        let op_store = borrow_global_mut<Pool>(store_addr);
        return (op_store.trueVotes*100/op_store.totalVotes*100)/100
    }


    public fun voteCount(store_addr: address):u64 acquires Pool{
        let op_store = borrow_global_mut<Pool>(store_addr);
        return op_store.totalVotes
    }

    #[test(admin = @my_addrx)]
    fun test_flow(admin: signer)acquires Pool {
